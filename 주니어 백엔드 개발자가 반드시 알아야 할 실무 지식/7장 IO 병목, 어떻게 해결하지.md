...
## 언제 어떤 방법을 택할까
논블러킹 IO나 가상 스레드를 적용할 때 검토해야할 사항
- 문제가 있는가?
- 문제가 있다면 네트워크 IO 관련 성능 문제인가?
- 구현 변경이 가능한가?

1. 성능 문제가 있는가? 존재하는가?
   - 존재하지 않는다면 논블러킹 IO나 가상 스레드를 검토할 필요가 없음
   - 논블러킹/비동기 IO는 코드의 복잡성을 높이고 유지보수 난이도가 올라간다. -> 단순 호기심으로 구현 변경을 시도하지 말 것
2. 성능 문제가 있다면 원인을 파악하자
   - 쿼리 시간이 길다면 가상 스레드나 논블리컹 IO를 적용해도 응답 시간을 줄일 수 없음 -> 쿼리 최적화나 캐시를 이용
   - CPU 중심 작업이도 마찬가지
   - IO 관련한 문제라면 구현 변경이 가능한지 여부를 따져본다.
     - 적용할 수 있다면, 가상 스레드나 논블러킹 IO 사용
     - 없다면 메모리 늘리거나 서버 수평 확장
     - 혹은 기술에 익숙한지 여부도 함께 고려할 것
